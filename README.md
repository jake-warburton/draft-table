...
Draft Table is a planned public, free-to-host multiplayer draft simulator for Flesh and Blood. The MVP is scoped to **Omens of the Third Age**, with 1–8 human drafters and up to 16 total room participants including spectators.
...
Initialize happens exactly once. The first request creates the room and answers with its code and a one-time **host claim**; any later attempt gets the one *taken* answer the router acts on. The creator's configuration is read strictly — an optional password, timers, pool visibility, spectators, and seat randomization, with the defaults the [product contract](docs/product.md#room-configuration) names; rooms have no chosen names, because the minted code is the room's whole identity — and an unknown field or unsound value refuses the whole request, leaving no room behind. The refusal reaches the caller in the router's own words; neither side's body is repeated.

Secrets are stored only as salted digests. The room password and the host claim each become a verifier, compared in constant time; the claim's plaintext leaves the object exactly once, inside the creation answer, and is how the creator's first socket proves it is the host. The deployment-secret pepper the [security notes](docs/security-and-privacy.md#passwords-and-invitation-links) call for joins when a deployment exists to hold the secret.

A socket is nobody until its hello. The upgrade is accepted silently, and the first message must be a well-formed hello carrying the optional password, an optional returning credential, an optional display name, and — once, for the creator — the host claim. The gate refuses in one generic shape: wrong and missing passwords answer identically, a refusal carries no state and closes the door, and nothing mutates and no room data leaves before verification. A successful hello mints a 256-bit identity credential, returned once and stored only as a verifier; a returning credential reclaims its identity and seat, and the newest connection for an identity wins — the older socket is closed, and a generation guard keeps that older socket's closing from marking the newer one dead. Joins fill the eight numbered seats first and then the spectator row, up to sixteen participants. Display names are validated as plain visible text, and default names never collide.

The canonical snapshot is persisted before anything is answered or broadcast, and every handler runs whole inside the object's concurrency gate: the platform holds new events only across storage waits, and these handlers also wait on crypto between reading the snapshot and writing it back, so the gate is what actually delivers the protocol's promise that the room serializes all commands — two racing joins land one at a time, and the host claim cannot be double-spent however close the race.

The object's one alarm serves two appointments. While the abandonment deadline is recorded — thirty minutes after creation, or after the lobby's last goodbye — a lobby with nobody connected at that deadline is deleted whole. While anyone is believed connected, the appointment is a standing liveness sweep instead: a deploy tears every socket down without a close event, so the sweep checks the lobby's belief against the sockets that actually exist, marks the ghosts disconnected, and starts the abandonment clock if nobody real remains — a lobby cannot leak forever on a swallowed goodbye. Alarms are at-least-once and carry no trusted data, so the handler re-reads canonical storage and stays harmless when it fires early, late, or twice — an early wake rebooks the appointment rather than losing it.

The lobby answers its commands. A drafter renames themselves; the host reshapes the safe options as one whole object, moves and swaps seats or sends a guest to the spectator row, shuffles the seated with a server-owned **Randomize now**, re-arms or disarms the start-time shuffle, and removes a guest — never the host. The first manual move visibly spends the pending shuffle, exactly as the product contract promises. A guest who leaves opens their slot, and their credential dies with their place; when the last participant leaves on purpose, the room is deleted on the spot. Commands out of place get structured answers — the wrong role is refused, the wrong phase is refused — and `resync` always answers with a fresh snapshot.

The draft itself runs inside the room. The host's start is checked against the exact room they were looking at, applies any pending seat shuffle, compacts the occupied positions into the ring, and deals every pack of all three rounds from the reviewed snapshot — the draft's whole future is decided and persisted before anyone hears it began. Picks are provisional and replaceable: a drafter queues a card, the table learns only *that* the seat queued, and the queued identity never enters a broadcast. Deadlines follow the official judge schedule by pack size; when every connected drafter has queued, the pick closes in five seconds — a floor no replacement can extend, which is also what makes the confirmation one-shot. With timers off, nothing ticks until that confirmation. The deadline commits the queued and draws uniformly for the silent — empty and disconnected seats included, which is timeout resolution, not a bot — then packs pass, the single last card assigns itself, and between packs the table holds a one-minute review with the pile face up, exactly as the solo table does. A finished room stays readable for an hour, then everything is deleted.

Disconnection never vacates a seat: the seat notes the absence, deadlines keep running, and a returning credential finds its pack waiting where it left it. A stranger arriving mid-draft is a spectator where spectators are allowed, and nothing at all where they are not. The room's own pool-hiding option governs the pile between reviews, and each drafter's private view carries their pack, their queued card, and nobody else's.
...
The table lays the pack out five cards to a row, so a fresh fourteen-card pack reads as three rows of five, five, and four. Drafted cards stack into one pile per group: every card but the newest shows only its top tenth, so the pile reads as a run of name bars, and the newest card sits on top of the pile in full. A card whose art fails keeps its place in the pile and shows its name as text instead.

While you are picking, the pile is face down: the pool region shows a **deck of drawn card backs, one per drafted card**, each stepped a couple of pixels so the pile visibly thickens pick by pick — and the cards themselves are genuinely absent rather than hidden with styling; the deck is a count made physical, carrying no card data. The official card back is not served by the one origin the page may load images from, so the backs are drawn, not fetched. The count in the heading stays honest, and in a room it is derived from the public pick number. When a pack runs out, the table pauses for a **pack review** under a *Pack 1 review* or *Pack 2 review* heading: the pile turns face up, the next pass direction is announced, and the next pack waits until you continue. The finished draft turns the pile face up for good. The one-minute review timer in the UX notes needs a server to hold every seat to the same clock, so it waits for multiplayer rooms.
