...
Neither evidence command commits or configures its input files. Draft-04 validation is explicit build-time tooling: import `@draft-table/set-omens/schema-validation` for it; the normal `@draft-table/set-omens` root intentionally does not load its Ajv dependencies.
