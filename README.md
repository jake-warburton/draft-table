...
>
> `apps/web` is a readable, dependency-free single-player draft client. It runs the real `packages/draft` transitions and the real `packages/engine` unbiased mapper in the browser: eight seats (you plus seven bots), three rounds of fourteen-card packs, thirteen choices per round, left/right/left passing, and a forty-two-card pool. Its card material is **placeholder only** — an obviously invented catalogue in `apps/web/src/cards.ts` that mirrors the recipe layout shape (eleven common-rarity positions, one fixed rare, one rare-or-majestic, one Rainbow Foil) so the reviewed Omens set snapshot can replace the catalogue without touching the draft. The browser owns entropy through `crypto.getRandomValues`; no transition generates its own randomness. `npm run size` reports completed `apps/web/dist` bytes and rejects missing output; there is no client byte ceiling, `clientCeiling`, size gate, or retained minifier cap. The removed cap had forced accessibility and responsive declarations out, so a budget cannot decide product quality accidentally. Real cards, multiplayer rooms, networking, deployment, Fabrary integration, and timers have not begun.
