...
> **Status: walking skeleton implementation begun.** Phase 2 checksum verification plus strict Settings-envelope, CustomCards-schema, and indentation-sensitive Layouts-schema parsing are implemented. Pinned layout aggregate/coefficient fixtures, pool resolution and parsing, source reconciliation, generation, import, and snapshots remain unimplemented. The repository otherwise contains only a neutral browser shell and workspace boundaries. Draft behavior, rooms, networking, deployment, Fabrary integration, and product UI have not begun.
