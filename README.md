...
Pull requests and pushes to `main` install with `npm ci` and run every quality command listed above. `npm run size` enforces a 2,048-byte total emitted-client ceiling; the server boundary is not emitted yet and reports as zero bytes. The current client size is recorded in [the web playable shell note](docs/streams/web-playable-shell.md).
