...
The canonical snapshot is persisted before anything is answered or broadcast, and the object's one alarm serves the abandonment rule: thirty minutes after creation, or after the lobby's last disconnect, a lobby with nobody connected is deleted whole. The recorded deadline is the single authority — it is set only while nobody is connected and any join clears it. Alarms are at-least-once and carry no trusted data, so the handler re-reads canonical storage and stays harmless when it fires early, late, or twice — an early wake rebooks the appointment rather than losing it.
