...
Neither evidence command commits or configures its input files. Draft-04 validation and the resulting OMN source projection are explicit build-time tooling: import `validateFabEnglishCardDataAgainstSchema` and `projectSchemaValidatedFabEnglishCardDataForOmn` from `@draft-table/set-omens/schema-validation`. That subpath requires the package-local Ajv development dependencies and may fail when they are unavailable; the normal `@draft-table/set-omens` root does not load them.
