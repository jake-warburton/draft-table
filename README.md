...
Not there yet: the start of the draft itself — pack generation, picks, deadlines, and the one-minute reviews — and password changes after creation. The object's storage, time, entropy, and socket plumbing arrive through narrow injected slices, so its tests drive the whole gate without a Cloudflare runtime.
