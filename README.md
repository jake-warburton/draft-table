...
Pull requests and pushes to `main` install with `npm ci` and run every quality command listed above. See [the web playable shell note](docs/streams/web-playable-shell.md#build-ownership-and-evidence) for the `npm run size` reporting contract; the server boundary is not emitted yet and reports as zero bytes.
