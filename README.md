...
Not there yet: the lobby's commands — renaming, seat moves, configuration changes, leaving — and the start of the draft itself. The object's storage, time, entropy, and socket plumbing arrive through narrow injected slices, so its tests drive the whole gate without a Cloudflare runtime.
