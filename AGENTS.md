...
- The v8.2.0 public English card input and schema are separately checksum-pinned build-time inputs; use `npm --workspace @draft-table/set-omens run test:public-source-evidence` only with caller-provided local evidence paths, never commit upstream bytes or add runtime fetches.
