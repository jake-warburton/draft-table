...
import { discoverPackLocalPoolTicketSelectionEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Pack local pool ticket selection acceptance failed."); process.exit(1); };
...
const files = discoverPackLocalPoolTicketSelectionEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("pack local pool ticket selection acceptance passed");
