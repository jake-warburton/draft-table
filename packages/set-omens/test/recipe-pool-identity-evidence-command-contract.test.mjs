...
const runner = fileURLToPath(new URL("./recipe-pool-identity-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources resolve every validated recipe pool entry to its exact draftable official identity";
const marker = "RECIPE_POOL_IDENTITY_RESOLUTION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic recipe pool identity command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-recipe-pool-identity-command-")); const testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.recipe-pool-identity-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Recipe pool identity resolution acceptance failed.\n"); };

test("recipe pool identity command rejects every missing, unreadable, or checksum-wrong evidence variable", () => {
...
test("recipe pool identity command rejects usage errors and arbitrary nonzero exits", () => {
...
test("recipe pool identity command rejects skipped, nonexecuted, wrong-marker, unrelated-failure, and duplicate contracts", () => {
  ...
  rejected(run(passing.replace(marker, "WRONG_RECIPE_POOL_IDENTITY_MARKER")));
...
test("recipe pool identity command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "recipe pool identity resolution acceptance passed\n"); assert.equal(result.stderr, "");
});
