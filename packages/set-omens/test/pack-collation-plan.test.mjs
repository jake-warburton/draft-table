...
const errorFieldsContract = "plan error defines stable own fields without prototype assignment", errorFieldsMarker = "PLAN_ERROR_OWN_FIELDS_CONTRACT_EXECUTED";
test(errorFieldsContract, async () => { console.log(errorFieldsMarker); const { plan } = await mutationModules(); const originalName = Object.getOwnPropertyDescriptor(Error.prototype, "name"); try { Object.defineProperty(Error.prototype, "name", { configurable: true, set() { throw new Error("PLAN_ERROR_FIELDS_MUST_BYPASS_PROTOTYPE_SETTERS"); } }); const error = new plan.OmensPackCollationPlanInitializationError(); assert.ok(error instanceof plan.OmensPackCollationPlanInitializationError); assert.equal(error.name, "OmensPackCollationPlanInitializationError"); assert.equal(error.code, "OMENS_PACK_COLLATION_PLAN_INITIALIZATION_FAILED"); assert.equal(error.message, "Omens pack collation plan initialization failed."); assert.equal(error.stack, "OmensPackCollationPlanInitializationError: Omens pack collation plan initialization failed."); } finally { Object.defineProperty(Error.prototype, "name", originalName); } });
test("plan error own-field semantic mutation fails its exact named contract", () => {
  const original = readFileSync(sourcePath, "utf8"), anchor = 'defineOwnDataProperty(this, "name", { value: "OmensPackCollationPlanInitializationError", writable: true, enumerable: true, configurable: true });', mutated = original.replace(anchor, 'this.name = "OmensPackCollationPlanInitializationError";'); assert.equal(original.split(anchor).length - 1, 1); assert.notEqual(mutated, original);
  let directory; try { directory = mkdtempSync(join(tmpdir(), "draft-table-plan-error-fields-")); const sourceDirectory = fileURLToPath(new URL("../src/", import.meta.url)); for (const file of readdirSync(sourceDirectory).filter((file) => file.endsWith(".ts"))) copyFileSync(join(sourceDirectory, file), join(directory, file)); symlinkSync(join(sourceDirectory, "../../../node_modules"), join(directory, "node_modules"), "dir"); writeFileSync(join(directory, "pack-collation-plan.ts"), mutated); writeFileSync(join(directory, "tsconfig.json"), '{"compilerOptions":{"target":"ES2022","module":"ES2022","moduleResolution":"bundler","strict":true,"noEmit":true,"allowImportingTsExtensions":true},"include":["*.ts"]}'); const typecheck = spawnSync(join(directory, "node_modules", ".bin", "tsc"), ["-p", join(directory, "tsconfig.json")], { encoding: "utf8" }); assert.equal(typecheck.status, 0, `${typecheck.stdout}\n${typecheck.stderr}`); const environment = { ...process.env, [mutationModuleKey]: pathToFileURL(join(directory, "pack-collation-plan.ts")).href }; delete environment.NODE_TEST_CONTEXT; const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", "--test-name-pattern", exactTestNamePattern(errorFieldsContract), fileURLToPath(import.meta.url)], { encoding: "utf8", env: environment }), lines = result.stdout.split(/\r?\n/u); assert.equal(result.status, 1, `${result.stdout}\n${result.stderr}`); assert.equal(lines.filter((line) => line === `# ${errorFieldsMarker}`).length, 1); assert.equal(lines.filter((line) => /^not ok \d+ - /u.test(line) && line.includes(errorFieldsContract)).length, 1); assert.equal(lines.filter((line) => line.includes("PLAN_ERROR_FIELDS_MUST_BYPASS_PROTOTYPE_SETTERS")).length, 1); } finally { if (directory !== undefined) rmSync(directory, { recursive: true, force: true }); }
});

const errorFreezeContract = "plan error freezes constructor and prototype while preserving instanceof", errorFreezeMarker = "PLAN_ERROR_FREEZE_CONTRACT_EXECUTED";
test(errorFreezeContract, async () => { console.log(errorFreezeMarker); const { plan } = await mutationModules(); assert.ok(Object.isFrozen(plan.OmensPackCollationPlanInitializationError), "PLAN_ERROR_CONSTRUCTOR_MUST_BE_FROZEN"); assert.ok(Object.isFrozen(plan.OmensPackCollationPlanInitializationError.prototype), "PLAN_ERROR_PROTOTYPE_MUST_BE_FROZEN"); assert.throws(() => Object.defineProperty(plan.OmensPackCollationPlanInitializationError, Symbol.hasInstance, { value: () => false }), TypeError); assert.ok(new plan.OmensPackCollationPlanInitializationError() instanceof plan.OmensPackCollationPlanInitializationError); });
test("plan error freeze semantic mutation fails its exact named contract", () => {
  const original = readFileSync(sourcePath, "utf8"), anchor = "freeze(OmensPackCollationPlanInitializationError.prototype);\nfreeze(OmensPackCollationPlanInitializationError);", mutated = original.replace(anchor, "void OmensPackCollationPlanInitializationError.prototype;\nvoid OmensPackCollationPlanInitializationError;"); assert.equal(original.split(anchor).length - 1, 1); assert.notEqual(mutated, original);
  let directory; try { directory = mkdtempSync(join(tmpdir(), "draft-table-plan-error-freeze-")); const sourceDirectory = fileURLToPath(new URL("../src/", import.meta.url)); for (const file of readdirSync(sourceDirectory).filter((file) => file.endsWith(".ts"))) copyFileSync(join(sourceDirectory, file), join(directory, file)); symlinkSync(join(sourceDirectory, "../../../node_modules"), join(directory, "node_modules"), "dir"); writeFileSync(join(directory, "pack-collation-plan.ts"), mutated); writeFileSync(join(directory, "tsconfig.json"), '{"compilerOptions":{"target":"ES2022","module":"ES2022","moduleResolution":"bundler","strict":true,"noEmit":true,"allowImportingTsExtensions":true},"include":["*.ts"]}'); const typecheck = spawnSync(join(directory, "node_modules", ".bin", "tsc"), ["-p", join(directory, "tsconfig.json")], { encoding: "utf8" }); assert.equal(typecheck.status, 0, `${typecheck.stdout}\n${typecheck.stderr}`); const environment = { ...process.env, [mutationModuleKey]: pathToFileURL(join(directory, "pack-collation-plan.ts")).href }; delete environment.NODE_TEST_CONTEXT; const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", "--test-name-pattern", exactTestNamePattern(errorFreezeContract), fileURLToPath(import.meta.url)], { encoding: "utf8", env: environment }), lines = result.stdout.split(/\r?\n/u); assert.equal(result.status, 1, `${result.stdout}\n${result.stderr}`); assert.equal(lines.filter((line) => line === `# ${errorFreezeMarker}`).length, 1); assert.equal(lines.filter((line) => /^not ok \d+ - /u.test(line) && line.includes(errorFreezeContract)).length, 1); assert.equal(lines.filter((line) => line.includes("PLAN_ERROR_CONSTRUCTOR_MUST_BE_FROZEN")).length, 1); } finally { if (directory !== undefined) rmSync(directory, { recursive: true, force: true }); }
});
...
const capacityContract = "plan capacity validation rejects an exact owned layout before initialization", capacityMarker = "PLAN_CAPACITY_PREINITIALIZATION_CONTRACT_EXECUTED";
test(capacityContract, async () => {
  console.log(capacityMarker); const { plan, drawState, capabilities } = await mutationModules(true), layout = capabilities.tables.layoutChoices[0].layoutReference, pool = capabilities.tables.poolTables[0]; let initialized = 0;
