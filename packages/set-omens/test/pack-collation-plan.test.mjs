...
const mutationModules = async () => { const moduleUrl = process.env[mutationModuleKey] ?? sourcePath.href, directory = new URL("./", moduleUrl); const [plan, custom, eligibility, layouts, upstream, pools, identity, layoutResolution, poolResolution, compiler] = await Promise.all([import(moduleUrl), import(new URL("custom-cards.ts", directory)), import(new URL("draft-eligibility-classification.ts", directory)), import(new URL("layouts.ts", directory)), import(new URL("official-upstream-id-reconciliation.ts", directory)), import(new URL("pools.ts", directory)), import(new URL("recipe-official-identity-reconciliation.ts", directory)), import(new URL("recipe-layout-pool-resolution.ts", directory)), import(new URL("recipe-pool-identity-resolution.ts", directory)), import(new URL("collation-weight-tables.ts", directory))]); return { plan, capabilities: fictionalCollationCapabilities({ ...custom, ...eligibility, ...layouts, ...upstream, ...pools, ...identity, ...layoutResolution, ...poolResolution, ...compiler }) }; };
...
const stateContract = "pack collation plan fresh state must come from its exact capability", stateMarker = "PACK_COLLATION_PLAN_STATE_CAPABILITY_CONTRACT_EXECUTED";
test(stateContract, async () => { console.log(stateMarker); const { plan, capabilities } = await mutationModules(), foreign = fictionalCollationCapabilities(); assert.throws(() => plan.initializeOmensPackCollationPlanFromOneUnsigned32SampleForTest(capabilities.tables, 0, (tables, sample) => ({ state: "selected", layoutReference: tables.layoutChoices[0].layoutReference }), () => initializeOmensPackLocalPoolDrawState(foreign.tables)), { code: "OMENS_PACK_COLLATION_PLAN_INITIALIZATION_FAILED" }, "FRESH_STATE_MUST_USE_EXACT_CAPABILITY"); });
test("plan foreign-state semantic mutation fails its exact named contract", () => { const original = readFileSync(sourcePath, "utf8"), mutated = original.replace("isOmensPackLocalPoolDrawStateRegisteredForPlanInitialization(tables, poolDrawState)", "Object.isFrozen(poolDrawState)"); assert.notEqual(mutated, original); runMutation(mutated, stateContract, stateMarker, "FRESH_STATE_MUST_USE_EXACT_CAPABILITY"); });
