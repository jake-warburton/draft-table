...
import { discoverDraftEligibilityEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Draft eligibility classification acceptance failed."); process.exit(1); };
...
const files = discoverDraftEligibilityEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("draft eligibility classification acceptance passed");
