...
import { discoverPackCollationPlanSelectionHistoryEvidenceTests } from "./evidence-discovery.mjs";
...
const contractName = "four checksum-verified caller-held sources retain every Omens plan selection in exact source order";
const marker = "PACK_COLLATION_PLAN_SELECTION_HISTORY_CONTRACT_EXECUTED";
const fail = () => { console.error("Pack collation plan selection history acceptance failed."); process.exit(1); };
...
const files = discoverPackCollationPlanSelectionHistoryEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("pack collation plan selection history acceptance passed");
