...
const runner = fileURLToPath(new URL("./draft-eligibility-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources establish the captain-approved Omens draft eligibility classification";
const marker = "DRAFT_ELIGIBILITY_CLASSIFICATION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic draft eligibility command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-draft-eligibility-command-")); const testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.draft-eligibility-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Draft eligibility classification acceptance failed.\n"); };

test("draft eligibility command rejects every missing, unreadable, or checksum-wrong evidence variable", () => {
...
test("draft eligibility command rejects usage errors and arbitrary nonzero exits", () => {
  rejected(run(passing, { args: ["unexpected"] }));
  rejected(run(passing.replace(`console.log(${JSON.stringify(marker)});`, `console.log(${JSON.stringify(marker)}); process.exitCode = 19;`)));
});
test("draft eligibility command rejects skipped, nonexecuted, wrong-marker, unrelated-failure, and duplicate contracts", () => {
  ...
  rejected(run(passing.replace(marker, "WRONG_DRAFT_ELIGIBILITY_MARKER")));
...
test("draft eligibility command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "draft eligibility classification acceptance passed\n"); assert.equal(result.stderr, "");
});
