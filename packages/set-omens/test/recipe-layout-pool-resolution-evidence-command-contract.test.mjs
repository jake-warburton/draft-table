...
const marker = "RECIPE_LAYOUT_POOL_RESOLUTION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic recipe layout pool resolution command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-recipe-layout-pool-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.recipe-layout-pool-resolution-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Recipe layout pool resolution acceptance failed.\n"); };

test("recipe layout pool resolution command rejects every missing, unreadable, or checksum-wrong evidence variable", () => {
...
test("recipe layout pool resolution command rejects usage errors and arbitrary nonzero exits", () => {
