...
import { discoverRecipePoolIdentityEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Recipe pool identity resolution acceptance failed."); process.exit(1); };
...
const files = discoverRecipePoolIdentityEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("recipe pool identity resolution acceptance passed");
