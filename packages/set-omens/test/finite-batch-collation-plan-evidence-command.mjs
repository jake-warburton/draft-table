...
import { discoverFiniteBatchCollationPlanEvidenceTests } from "./evidence-discovery.mjs";
...
const contractName = "four checksum-verified caller-held sources initialize every Omens layout-bound fresh plan from finite batches";
const marker = "FINITE_BATCH_COLLATION_PLAN_INITIALIZATION_CONTRACT_EXECUTED";
const fail = () => { console.error("Finite batch collation plan initialization acceptance failed."); process.exit(1); };
...
const files = discoverFiniteBatchCollationPlanEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("finite batch collation plan initialization acceptance passed");
