...
import { discoverPackLocalPoolSampleDrawTransitionEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Pack local sample draw transition acceptance failed."); process.exit(1); };
...
const files = discoverPackLocalPoolSampleDrawTransitionEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
const lines = result.stdout.split(/\r?\n/u), exactMarker = lines.filter((line) => line === `# ${marker}`).length === 1, exactContract = lines.filter((line) => /^ok \d+ - /u.test(line) && line.replace(/^ok \d+ - /u, "") === contractName).length === 1;
...
console.log("pack local sample draw transition acceptance passed");
