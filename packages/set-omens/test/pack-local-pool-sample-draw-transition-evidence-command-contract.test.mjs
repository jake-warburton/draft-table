...
const runner = fileURLToPath(new URL("./pack-local-pool-sample-draw-transition-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources atomically draw one uint32 sample and remove only its exact current pack-local identity";
const marker = "PACK_LOCAL_POOL_SAMPLE_DRAW_TRANSITION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic pack local sample draw transition command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-pack-local-pool-sample-draw-transition-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.pack-local-pool-sample-draw-transition-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Pack local sample draw transition acceptance failed.\n"); };

test("pack-local atomic draw command rejects every missing unreadable or checksum-wrong evidence variable", () => {
...
test("pack-local atomic draw command rejects usage errors nonexecution arbitrary failure and duplicate execution", () => {
  rejected(run(passing, { args: ["unexpected"] })); rejected(run(`import test from "node:test"; test(${JSON.stringify(contractName)}, { skip: true }, () => {});`));
  rejected(run(passing.replace(contractName, `prefix ${contractName}`))); rejected(run(passing.replace(marker, "WRONG_PACK_LOCAL_POOL_SAMPLE_DRAW_TRANSITION_MARKER")));
...
test("pack-local atomic draw exact targeting escapes regex metacharacters", () => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-pack-local-pool-sample-draw-transition-pattern-")), fictionalName = "fictional [pack] (ticket)+ contract?", path = join(directory, "pattern.test.mjs");
...
test("pack-local atomic draw command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "pack local sample draw transition acceptance passed\n"); assert.equal(result.stderr, "");
});
