...
const runner = fileURLToPath(new URL("./collation-sample-selection-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources compose one uint32 sample with every exact collation scope";
const marker = "COLLATION_SAMPLE_SELECTION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic collation sample selection command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-sample-selection-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.collation-sample-selection-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Collation sample selection acceptance failed.\n"); };

test("collation sample selection command rejects every missing unreadable or checksum-wrong evidence variable", () => {
...
test("collation sample selection command rejects usage errors nonexecution arbitrary failure and duplicate execution", () => {
  ...
  rejected(run(passing.replace(marker, "WRONG_COLLATION_SAMPLE_SELECTION_MARKER")));
...
test("collation sample selection exact targeting escapes regex metacharacters", () => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-sample-selection-pattern-")), fictionalName = "fictional [sample] (selection)+ contract?", path = join(directory, "pattern.test.mjs");
...
test("collation sample selection command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "collation sample selection acceptance passed\n"); assert.equal(result.stderr, "");
});
