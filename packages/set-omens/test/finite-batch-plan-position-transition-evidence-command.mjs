...
import { discoverFiniteBatchPlanPositionTransitionEvidenceTests } from "./evidence-discovery.mjs";
...
const contractName = "four checksum-verified caller-held sources transition every position of every Omens plan from finite batches";
const marker = "FINITE_BATCH_COLLATION_PLAN_POSITION_TRANSITION_CONTRACT_EXECUTED";
const fail = () => { console.error("Finite batch collation plan position transition acceptance failed."); process.exit(1); };
...
const files = discoverFiniteBatchPlanPositionTransitionEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("finite batch collation plan position transition acceptance passed");
