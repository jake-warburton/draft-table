...
const runner = fileURLToPath(new URL("./collation-weight-ticket-selection-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources exhaustively select source-order integer collation weights";
const marker = "COLLATION_WEIGHTED_SELECTION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic collation weighted selection command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-weighted-selection-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.collation-weight-ticket-selection-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Collation weighted selection acceptance failed.\n"); };

test("collation weighted selection command rejects every missing unreadable or checksum-wrong evidence variable", () => {
...
test("collation weighted selection command rejects usage errors nonexecution and arbitrary failure", () => {
  ...
  rejected(run(passing.replace(marker, "WRONG_COLLATION_WEIGHTED_SELECTION_MARKER")));
...
test("collation weighted selection exact targeting escapes regex metacharacters", () => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-weighted-selection-pattern-")), fictionalName = "fictional [weighted] (selection)+ contract?", path = join(directory, "pattern.test.mjs");
...
test("collation weighted selection command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "collation weighted selection acceptance passed\n"); assert.equal(result.stderr, "");
});
