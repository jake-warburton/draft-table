...
import { discoverCollationWeightTablesEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Collation weight tables acceptance failed."); process.exit(1); };
...
const files = discoverCollationWeightTablesEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("collation weight tables acceptance passed");
