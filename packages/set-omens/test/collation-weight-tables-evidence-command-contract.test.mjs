...
const runner = fileURLToPath(new URL("./collation-weight-tables-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources compile exact source-order integer collation weight tables";
const marker = "COLLATION_WEIGHT_TABLES_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic collation weight tables command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-weight-tables-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.collation-weight-tables-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Collation weight tables acceptance failed.\n"); };

test("collation weight tables command rejects every missing, unreadable, or checksum-wrong evidence variable", () => {
...
test("collation weight tables command rejects usage errors and arbitrary nonzero exits", () => {
...
test("collation weight tables command rejects skipped, nonexecuted, wrong-marker, unrelated-failure, and duplicate contracts", () => {
  ...
  rejected(run(passing.replace(marker, "WRONG_COLLATION_WEIGHT_TABLES_MARKER")));
...
test("collation weight tables exact targeting escapes regex metacharacters", () => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-collation-weight-tables-pattern-"));
  const fictionalName = "fictional [collation] (weight)+ contract?", path = join(directory, "pattern.test.mjs");
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr); assert.match(result.stdout, /^# FICTIONAL_CONTRACT_EXECUTED$/m); assert.match(result.stdout, /^# tests 1$/m); assert.match(result.stdout, /^# pass 1$/m); assert.match(result.stdout, /^# skipped 0$/m); assert.doesNotMatch(result.stdout, /^ok \d+ - prefix /m);
  } finally { rmSync(directory, { recursive: true, force: true }); }
});
test("collation weight tables command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "collation weight tables acceptance passed\n"); assert.equal(result.stderr, "");
});
