...
test("MV equality/set semantic mutation is caught by its named contract", () => {
  ...
  assert.notEqual(mutated, original, "per-entry equality and set-equality guards present"); const path = `${dirname(fileURLToPath(sourcePath))}/mv-multiplicity-mutation-${process.pid}.ts`; writeFileSync(path, mutated);
  try { const env = { ...process.env, [equalityModuleKey]: pathToFileURL(path).href }; delete env.NODE_TEST_CONTEXT; const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", "--test-name-pattern", `^${equalityContract}$`, fileURLToPath(import.meta.url)], { encoding: "utf8", env }); const lines = result.stdout.split(/\r?\n/);
