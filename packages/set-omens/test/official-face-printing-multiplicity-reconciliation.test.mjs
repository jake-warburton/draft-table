...
const source = (counts = new Map(mv.map((entry) => [entry.id, entry.positions.length]))) => forms.map((form) => ({ unique_id: `card-${form.baseCollectorId}`, name: "fictional", pitch: "", printings: rowsFor(form, counts.get(form.officialPrintId) ?? (form.suffixMarker === "RF" ? 2 : 1)) }));
