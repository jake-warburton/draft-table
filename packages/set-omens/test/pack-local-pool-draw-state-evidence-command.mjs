...
import { discoverPackLocalPoolDrawStateEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Pack local pool draw state acceptance failed."); process.exit(1); };
...
const files = discoverPackLocalPoolDrawStateEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("pack local pool draw state acceptance passed");
