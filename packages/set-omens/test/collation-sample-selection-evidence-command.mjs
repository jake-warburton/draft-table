...
import { discoverCollationSampleSelectionEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Collation sample selection acceptance failed."); process.exit(1); };
...
const files = discoverCollationSampleSelectionEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("collation sample selection acceptance passed");
