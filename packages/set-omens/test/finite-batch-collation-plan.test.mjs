...
test("finite batch validates one captured length and exact mapper consumed-count ranges", () => {
  const { tables } = fictionalCollationCapabilities();
  let lengthReads = 0;
  const samples = new Proxy([0], { get(target, property, receiver) { if (property === "length") lengthReads++; return Reflect.get(target, property, receiver); } });
  assert.equal(initializeOmensPackCollationPlanFromUnsigned32SampleBatch(tables, samples).state, "selected");
  assert.equal(lengthReads, 2);
  safe(() => initializeOmensPackCollationPlanFromUnsigned32SampleBatch(tables, new Proxy([], { get(target, property, receiver) { if (property === "length") throw new Error("hostile length"); return Reflect.get(target, property, receiver); } })));
  safe(() => initializeOmensPackCollationPlanFromUnsigned32SampleBatchForTest(tables, [0], () => Object.freeze({ state: "accepted", ticket: 0, consumedSamples: 2 })));
  safe(() => initializeOmensPackCollationPlanFromUnsigned32SampleBatchForTest(tables, [0], () => Object.freeze({ state: "needs-sample", consumedSamples: 0 })));
});
...
const acceptedCountContract = "finite batch plan rejects accepted mapper counts beyond the supplied batch", acceptedCountMarker = "FINITE_BATCH_PLAN_ACCEPTED_COUNT_CONTRACT_EXECUTED";
test(acceptedCountContract, async () => { console.log(acceptedCountMarker); const moduleUrl = process.env[mutationModuleKey] ?? sourcePath.href, directory = new URL("./", moduleUrl), [plan, custom, eligibility, layouts, upstream, pools, identity, layoutResolution, poolResolution, compiler] = await Promise.all([import(moduleUrl), import(new URL("custom-cards.ts", directory)), import(new URL("draft-eligibility-classification.ts", directory)), import(new URL("layouts.ts", directory)), import(new URL("official-upstream-id-reconciliation.ts", directory)), import(new URL("pools.ts", directory)), import(new URL("recipe-official-identity-reconciliation.ts", directory)), import(new URL("recipe-layout-pool-resolution.ts", directory)), import(new URL("recipe-pool-identity-resolution.ts", directory)), import(new URL("collation-weight-tables.ts", directory))]), capabilities = fictionalCollationCapabilities({ ...custom, ...eligibility, ...layouts, ...upstream, ...pools, ...identity, ...layoutResolution, ...poolResolution, ...compiler }); assert.throws(() => plan.initializeOmensPackCollationPlanFromUnsigned32SampleBatchForTest(capabilities.tables, [0], () => Object.freeze({ state: "accepted", ticket: 0, consumedSamples: 2 })), { code: "OMENS_PACK_COLLATION_PLAN_INITIALIZATION_FAILED" }, "ACCEPTED_COUNT_MUST_NOT_EXCEED_BATCH"); });
test("finite batch accepted-count upper-bound mutation fails its exact named contract", () => {
  const anchor = "consumedSamples < 1 || consumedSamples > sampleCount", original = readFileSync(sourcePath, "utf8"), mutated = original.replace(anchor, "consumedSamples < 1 || false"); assert.equal(original.split(anchor).length - 1, 1); assert.notEqual(mutated, original);
  let directory; try { directory = mkdtempSync(join(tmpdir(), "draft-table-finite-batch-accepted-count-")); const sourceDirectory = fileURLToPath(new URL("../src/", import.meta.url)); for (const file of readdirSync(sourceDirectory).filter((file) => file.endsWith(".ts"))) copyFileSync(join(sourceDirectory, file), join(directory, file)); symlinkSync(join(sourceDirectory, "../../../node_modules"), join(directory, "node_modules"), "dir"); writeFileSync(join(directory, "finite-batch-collation-plan.ts"), mutated); writeFileSync(join(directory, "tsconfig.json"), '{"compilerOptions":{"target":"ES2022","module":"ES2022","moduleResolution":"bundler","strict":true,"noEmit":true,"allowImportingTsExtensions":true},"include":["*.ts"]}'); const typecheck = spawnSync(join(directory, "node_modules", ".bin", "tsc"), ["-p", join(directory, "tsconfig.json")], { encoding: "utf8" }); assert.equal(typecheck.status, 0, `${typecheck.stdout}\n${typecheck.stderr}`); const environment = { ...process.env, [mutationModuleKey]: pathToFileURL(join(directory, "finite-batch-collation-plan.ts")).href }; delete environment.NODE_TEST_CONTEXT; const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", "--test-name-pattern", exactPattern(acceptedCountContract), fileURLToPath(import.meta.url)], { encoding: "utf8", env: environment }), lines = result.stdout.split(/\r?\n/u); assert.equal(result.status, 1, `${result.stdout}\n${result.stderr}`); assert.equal(lines.filter((line) => line === `# ${acceptedCountMarker}`).length, 1); assert.equal(lines.filter((line) => /^not ok \d+ - /u.test(line) && line.includes(acceptedCountContract)).length, 1); assert.equal(lines.filter((line) => line.includes("ACCEPTED_COUNT_MUST_NOT_EXCEED_BATCH")).length, 1); } finally { if (directory !== undefined) rmSync(directory, { recursive: true, force: true }); }
});

const needsCountContract = "finite batch plan requires needs-sample to consume the supplied batch", needsCountMarker = "FINITE_BATCH_PLAN_NEEDS_COUNT_CONTRACT_EXECUTED";
test(needsCountContract, async () => { console.log(needsCountMarker); const moduleUrl = process.env[mutationModuleKey] ?? sourcePath.href, directory = new URL("./", moduleUrl), [plan, custom, eligibility, layouts, upstream, pools, identity, layoutResolution, poolResolution, compiler] = await Promise.all([import(moduleUrl), import(new URL("custom-cards.ts", directory)), import(new URL("draft-eligibility-classification.ts", directory)), import(new URL("layouts.ts", directory)), import(new URL("official-upstream-id-reconciliation.ts", directory)), import(new URL("pools.ts", directory)), import(new URL("recipe-official-identity-reconciliation.ts", directory)), import(new URL("recipe-layout-pool-resolution.ts", directory)), import(new URL("recipe-pool-identity-resolution.ts", directory)), import(new URL("collation-weight-tables.ts", directory))]), capabilities = fictionalCollationCapabilities({ ...custom, ...eligibility, ...layouts, ...upstream, ...pools, ...identity, ...layoutResolution, ...poolResolution, ...compiler }); assert.throws(() => plan.initializeOmensPackCollationPlanFromUnsigned32SampleBatchForTest(capabilities.tables, [0], () => Object.freeze({ state: "needs-sample", consumedSamples: 0 })), { code: "OMENS_PACK_COLLATION_PLAN_INITIALIZATION_FAILED" }, "NEEDS_COUNT_MUST_EQUAL_BATCH"); });
test("finite batch needs-sample count-equality mutation fails its exact named contract", () => {
  const anchor = "if (consumedSamples !== sampleCount) return fail();", original = readFileSync(sourcePath, "utf8"), mutated = original.replace(anchor, "if (false) return fail();"); assert.equal(original.split(anchor).length - 1, 1); assert.notEqual(mutated, original);
  let directory; try { directory = mkdtempSync(join(tmpdir(), "draft-table-finite-batch-needs-count-")); const sourceDirectory = fileURLToPath(new URL("../src/", import.meta.url)); for (const file of readdirSync(sourceDirectory).filter((file) => file.endsWith(".ts"))) copyFileSync(join(sourceDirectory, file), join(directory, file)); symlinkSync(join(sourceDirectory, "../../../node_modules"), join(directory, "node_modules"), "dir"); writeFileSync(join(directory, "finite-batch-collation-plan.ts"), mutated); writeFileSync(join(directory, "tsconfig.json"), '{"compilerOptions":{"target":"ES2022","module":"ES2022","moduleResolution":"bundler","strict":true,"noEmit":true,"allowImportingTsExtensions":true},"include":["*.ts"]}'); const typecheck = spawnSync(join(directory, "node_modules", ".bin", "tsc"), ["-p", join(directory, "tsconfig.json")], { encoding: "utf8" }); assert.equal(typecheck.status, 0, `${typecheck.stdout}\n${typecheck.stderr}`); const environment = { ...process.env, [mutationModuleKey]: pathToFileURL(join(directory, "finite-batch-collation-plan.ts")).href }; delete environment.NODE_TEST_CONTEXT; const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", "--test-name-pattern", exactPattern(needsCountContract), fileURLToPath(import.meta.url)], { encoding: "utf8", env: environment }), lines = result.stdout.split(/\r?\n/u); assert.equal(result.status, 1, `${result.stdout}\n${result.stderr}`); assert.equal(lines.filter((line) => line === `# ${needsCountMarker}`).length, 1); assert.equal(lines.filter((line) => /^not ok \d+ - /u.test(line) && line.includes(needsCountContract)).length, 1); assert.equal(lines.filter((line) => line.includes("NEEDS_COUNT_MUST_EQUAL_BATCH")).length, 1); } finally { if (directory !== undefined) rmSync(directory, { recursive: true, force: true }); }
});
