...
import { discoverCollationWeightedSelectionEvidenceTests } from "./evidence-discovery.mjs";
...
const fail = () => { console.error("Collation weighted selection acceptance failed."); process.exit(1); };
...
const files = discoverCollationWeightedSelectionEvidenceTests(readdirSync("test")).map((file) => `test/${file}`);
...
console.log("collation weighted selection acceptance passed");
