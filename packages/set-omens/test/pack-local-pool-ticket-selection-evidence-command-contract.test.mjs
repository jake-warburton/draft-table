...
const runner = fileURLToPath(new URL("./pack-local-pool-ticket-selection-evidence-command.mjs", import.meta.url));
...
const contractName = "four checksum-verified caller-held sources enforce dynamic pack-local ticket selection across every remaining identity-pool entry";
const marker = "PACK_LOCAL_POOL_TICKET_SELECTION_CONTRACT_EXECUTED";
const bytes = variables.map((_, index) => `synthetic pack local pool ticket selection command bytes ${index}`);
const passing = `
...
const run = (contract = passing, options = {}) => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-pack-local-pool-ticket-selection-command-")), testDirectory = join(directory, "test"); mkdirSync(testDirectory);
  writeFileSync(join(testDirectory, "probe.pack-local-pool-ticket-selection-evidence.test.mjs"), contract);
...
const rejected = (result) => { assert.notEqual(result.status, 0); assert.equal(result.stdout, ""); assert.equal(result.stderr, "Pack local pool ticket selection acceptance failed.\n"); };

test("pack-local ticket selection command rejects every missing unreadable or checksum-wrong evidence variable", () => {
...
test("pack-local ticket selection command rejects usage errors nonexecution arbitrary failure and duplicate execution", () => {
  rejected(run(passing, { args: ["unexpected"] })); rejected(run(`import test from "node:test"; test(${JSON.stringify(contractName)}, { skip: true }, () => {});`));
  rejected(run(passing.replace(contractName, `prefix ${contractName}`))); rejected(run(passing.replace(marker, "WRONG_PACK_LOCAL_POOL_TICKET_SELECTION_MARKER")));
...
test("pack-local ticket selection exact targeting escapes regex metacharacters", () => {
  const directory = mkdtempSync(join(tmpdir(), "draft-table-pack-local-pool-ticket-selection-pattern-")), fictionalName = "fictional [pack] (ticket)+ contract?", path = join(directory, "pattern.test.mjs");
...
test("pack-local ticket selection command sanitizes inherited context and prints only its exact success marker", () => {
  const result = run(); assert.equal(result.status, 0, result.stderr); assert.equal(result.stdout, "pack local pool ticket selection acceptance passed\n"); assert.equal(result.stderr, "");
});
