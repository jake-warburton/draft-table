...
- `packages/set-omens`: Omens-specific build-time source verification, strict recipe parsing, aggregate validation, source reconciliation, pool handling, import, and reviewed versioned snapshots. Its normal root is runtime-safe; public-source validation, projection, reconciliation, and suffix/foiling correspondence tooling is available only through the explicit build-time `@draft-table/set-omens/schema-validation` subpath.
