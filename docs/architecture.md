...
- `packages/set-omens`: Omens-specific build-time source identity, checksum, and strict Settings-envelope boundary; reviewed generated set snapshot and visible-recipe output/import evidence follow only after separate review.
- `apps/web`: static browser client.
- `apps/server`: thin Worker router and Durable Object adapter.

Only the neutral browser shell plus the Omens checksum and strict Settings-envelope boundaries are implemented; card/layout/pool parsing and import remain unimplemented, and the other boundaries remain empty. Keep the server plain rather than adding a general web framework. Select a small client rendering approach only after a bundle/accessibility spike; do not let a framework enter engine/contracts.
