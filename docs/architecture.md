...
- `packages/engine`: pure deterministic collation primitives plus an internal replay-only PCG source and bounded retry transcript; zero platform dependencies. PCG is non-cryptographic and not a multiplayer-unpredictability guarantee. Callers retain entropy ownership. Production stream separation, seed custody, and any cryptographic source remain a later architecture decision.
- `packages/contracts`: protocol/domain view contracts and boundary validation.
- `packages/set-omens`: Omens-specific build-time source verification, strict recipe parsing, aggregate validation, reconciliation, capability-bound 14-position pack construction, and reviewed versioned snapshots. Its build-time importer composes the engine primitives with opaque capabilities into complete finite-batch pack construction. Its normal root is runtime-safe. The explicit build-time `@draft-table/set-omens/schema-validation` subpath exposes source validation, reconciliation, and collation-plan tooling only; complete pack construction and card presentation remain package-internal build-time modules pending a reviewed supported export. The separate runtime-safe `@draft-table/set-omens/snapshot` subpath exposes the reviewed versioned set snapshot: identities, weighted pools, and weighted 14-position layouts, generated from the four pinned sources at build time, committed, and validated at load. It carries no evidence byte and loads no schema-validation dependency. It does carry each identity's exact official Card Vault image URL, pinned by `OMENS_SNAPSHOT_IMAGE_ORIGIN` to Legend Story Studios' own public host; no image byte is copied here and nothing in the package fetches one. Presentation remains source-attributed and build-time-only.
