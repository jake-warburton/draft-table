...
- `packages/engine`: pure deterministic draft/collation/state transitions and the one-sample uint32 rejection mapper; zero platform dependencies. Random-source ownership, retry loops, seed/state, collation integration, and packs remain outside this primitive.
