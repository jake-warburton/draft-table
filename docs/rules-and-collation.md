...
The build-time-only presentation projection accepts only capability-owned selected identity, upstream-printing, and Card Vault-face references. It copies attributed display name, pitch, rarity, treatment text, normal rendition URL, and identity/face boundaries without choosing any of them. Pitch colour and rear marker remain `null`; it does not infer treatment, rear, instance, collation, or runtime semantics. Public checksum/schema/membership evidence is required and no bytes are committed or fetched at runtime. Private four-source acceptance passed on PR 63 for exact head `4dbef5dd63c7e655f2cf9b4674d4238b0f1cac4b` (19/19, five stored exploits defeated, both named `defineProperty` live-global-substitution contracts failed, green CI; squash `34e81f3cf54e853ce8c07b9b4606416481a86ffa`).

The [README](../README.md) owns current implementation scope. Complete construction now sequences every 14-position layout with finite caller batches, immutable history/state, and no entropy ownership. It permits cross-pool normal/Rainbow-Foil identity overlap without deduplication: treatment distinguishes printings, and pitch variants are distinct identities. The guard proves normal pools are pairwise disjoint (8 pools/209 identities), RF pools are pairwise disjoint (3 pools/171 identities), and RF is a strict normal-union subset; it is structural evidence, not a cross-pool policy. Private four-source construction acceptance was accepted by the captain on PR 66: 20/20 and green CI for exact head `aa825b18b5c455d253af9c0c45842b0b21a4d4bd`, merged 2026-08-05T17:06:39Z (squash `bfda2d97104312d4276523ff23f683b296747b22`). The durable authority is the captain comment plus merge, because same-account GitHub approval is mechanically unavailable. Physical card instances, printing/treatment/image selection, excluded-rear wrapping, snapshots, runtime collation/room authority, and simulation remain future work. The bounded nine-IAR exclusion and 42 open unmapped identities are unchanged; neither excluded nor unclassified identities can enter through resolved pools.
