...
- accept the real recipe/upstream rarity, pool-identity, weighted-layout resolution, collation-weight table, bounded-ticket lookup, one-sample composition, pack-local same-pool removal, dynamic pack-local ticket selection, pack-local one-sample composition, atomic pack-local sample draw transition, layout-selected fresh collation-plan initialization, finite-batch collation-plan initialization, and finite-batch current-position transition results, then select reviewed physical treatments for those resolved identities;
