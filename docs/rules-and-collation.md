...
The completed collation-weight table compiler consumes only that exact registered layout capability and its exact registered pool capability through narrow readers. Its deeply immutable, copy-independent output retains the exact source-order layout and pool references, official-identity references, positive safe-integer weights, and integer `cumulativeExclusiveEnd` values, with a scoped total for the 228 layouts (460,800) and each of the 11 pools. It rejects copied, forged, foreign, mismatched, missing, or duplicate capability facts; invalid or overflowing weights and sums; source-order or aggregate drift; and caller overrides through one source-secret failure. Exact bounded-ticket lookup now consumes only that registered collation capability through separate layout and named-pool readers, returns only its immutable reference, and rejects every invalid ticket or ownership scope through one source-secret failure. The real four-source compilation and lookup acceptance remain captain-acceptance-pending.

Randomness, unbiased bounded random generation, seed/state, card draws, runtime no-replacement enforcement, cross-pool duplicate handling, pack construction, printing/treatment/image selection, excluded rear-slot wrapping, generated/versioned snapshots, runtime pools, runtime collation, and simulation remain unimplemented. The bounded nine-IAR exclusion and 42 open unmapped identities are unchanged; neither excluded nor unclassified identities can enter through the resolved pools.
...
- accept the real recipe/upstream rarity, pool-identity, weighted-layout resolution, and collation-weight table results, then select reviewed physical treatments for those resolved identities;
