...
The checksum-pinned v8.2.0/card-schema and dated Card Vault evidence observe nine identities with exact set code `IAR` in official Omens membership and eleven upstream IAR printing rows, all exact rarity `V`. The same reconciliation observes all nine IAR identities absent from the recipe while 209 recipe identities map. `V` is consistent with premium chase treatments, but observation of that code establishes neither slot nor draftability semantics.

Separately, the captain-supplied product context names the product and next set **i'Arathael** and states that `IAR` denotes it. It identifies the seven deadly-sin Runechants (Greed, Envy, Gluttony, Lust, Pride, Sloth, and Wrath), Baalghor, Omen of the End, and Gate to i'Arathael as an intentionally included omen/easter-egg and thematic product bridge, not a source defect. This context is not official publication provenance for the community recipe and cannot be re-verified by re-pinning the public evidence.
