...
8. Complete construction consumes only finite caller-supplied uint32 batches and preserves exact retry/accounting; its PCG helper is authorized for deterministic replay/known-answer behavior only, is non-cryptographic, and may be state-reconstructable from output.
9. Production seed custody, stream separation, and any cryptographic source remain a later architecture decision; no current helper implies competitive-multiplayer unpredictability.
10. A future production source must isolate pack, seat-order, and timeout-fallback streams so a timeout cannot alter pre-generated packs.
