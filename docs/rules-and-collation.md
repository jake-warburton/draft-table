...
- accept the real recipe/upstream rarity, pool-identity, weighted-layout resolution, collation-weight table, bounded-ticket lookup, one-sample composition, pack-local same-pool removal, and dynamic pack-local ticket-selection results, then select reviewed physical treatments for those resolved identities;
