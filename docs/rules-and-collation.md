...
The [README](../README.md) owns the current engine implementation scope. Random-source ownership, retry loops, deterministic seed/state, card draws, runtime no-replacement enforcement, cross-pool duplicate handling, ticket-to-collation integration, pack construction, printing/treatment/image selection, excluded rear-slot wrapping, generated/versioned snapshots, runtime pools, runtime collation, server authority, and simulation remain unimplemented. The bounded nine-IAR exclusion and 42 open unmapped identities are unchanged; neither excluded nor unclassified identities can enter through the resolved pools.
...
- accept the real recipe/upstream rarity, pool-identity, weighted-layout resolution, collation-weight table, and bounded-ticket lookup results, then select reviewed physical treatments for those resolved identities;
