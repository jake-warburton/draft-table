...
- accept the real recipe/upstream rarity, pool-identity, weighted-layout resolution, collation-weight table, bounded-ticket lookup, one-sample composition, and pack-local same-pool removal results, then select reviewed physical treatments for those resolved identities;
