...
The build-time-only presentation projection accepts only capability-owned selected identity, upstream-printing, and Card Vault-face references. It copies attributed display name, pitch, rarity, treatment text, normal rendition URL, and identity/face boundaries without choosing any of them. Pitch colour and rear marker remain `null`; it does not infer treatment, rear, instance, collation, or runtime semantics. Public checksum/schema/membership evidence is required, no bytes are committed or fetched at runtime, and private four-source acceptance remains NOT RUN pending captain measurement.
