...
The next build-time slice consumes that identity capability plus the opaque official/upstream reconciliation and examines only the 209 mapped candidates. It retains recipe collector text, unchanged recipe rarity label, official print ID, the full source-order rarity-code sequence with one exact value for every upstream printing row and duplicates preserved, a separate first-observed unique-code set used for per-identity correspondence classification, and an observed correspondence class. The captain-held four-source expectation for the authoritative mapped source-order sequences is `[C,C]`=117, `[R,R]`=59, `[M,M]`=15, `[C]`=15, `[C,V]`=2, and `[R]`=1: 209 identities total, of which 191 repeat a row code. The separately named lossy first-observed unique-code-set expectation is `{C}`=132, `{R}`=60, `{M}`=15, and `{C,V}`=2; in particular, the 132 `{C}` identities are exactly the 117 `[C,C]` plus 15 `[C]` sequences. These mapped sequence aggregates remain captain-acceptance-pending because public evidence can establish all 242 candidate sequences but cannot identify which 15 of the broader M-bearing candidates occur in the private mapping. The raw `mythic` label is retained rather than rewritten, while all project/FaB semantic vocabulary is Majestic and `M` remains only the observed upstream code. Public evidence independently establishes that each anomaly has one `C` row and one `V` row and no separately listed official suffix form; foiling and art-variation context is observation only, not a rarity gate. Both anomalies are explicitly flagged for later draftability/treatment classification. The pinned recipe's custom-card-list shape—fields such as `mana_cost` and `image_uris`, settings such as `showSlots`, `withReplacement`, and `cardBack`, and the source literal `mythic`—is strong circumstantial evidence of an MTG-drafting-tool format, but does not prove Draftmancer provenance. Ingest therefore preserves its exact MTG-style rarity vocabulary while one named exhaustive boundary maps source `mythic` to FaB-domain `majestic`; all unknown recipe labels remain generically rejected. Any other mixed identity, code-set drift, ownership drift, or aggregate redistribution fails closed. Checksum-pinned public evidence separately pins the broad upstream code domain `C|R|M|P|V|T|L|S|B|F`, 482 OMN official-base rows (C251/R119/M68/V18/B14/L10/F2), and 493 all-official retained rows; the latter composes from the OMN rows plus exactly 11 IAR `V` rows. The machinery is implemented, but the real four-source correspondence remains captain-acceptance-pending. These facts do not establish slot, treatment, exclusion, draftability, probability, or runtime semantics, and none of the 51 non-candidates is rarity-gated.
