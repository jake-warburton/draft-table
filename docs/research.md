...
The captain-supplied product context calls the next set **i'Arathael**; this player-recognizable product term is distinct from exact source text. For base collector `IAR222`, Card Vault's exact `printed_name` is `Gate to i'Arathael`, while v8.2.0 `card.json`'s exact `name` is `Gate to íArathael`. Exactly one of the 260 official names differs. Both strings are already NFC, so normalization cannot reconcile i-acute with apostrophe; future joins must remain collector-ID-first rather than name-only. The detailed IAR product/draft-legality boundary, including the nine identities and eleven `V` rows, is recorded in [the rules and collation contract](rules-and-collation.md#iar-product-context-and-name-boundary).
