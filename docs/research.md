...
Recommended inputs are the tagged English `json/english/card.json`, `set.json`, schemas, and official Card Vault product membership—not `develop` and not a live fetch at application runtime. The build-time boundary checksum-pins that card input and `json-schema/card-schema.json` to one immutable v8.2.0 commit, then strictly validates their JSON encoding/envelope, duplicate keys, root shape, and full pinned Draft-04 conformance after both checksums complete. It now projects the validated public card fields for exact `set_id === "OMN"` rows, while a separate strict Card Vault path establishes canonical official product membership (including the nine IAR entries). The implemented official/upstream and recipe identity reconciliation scope—and the remaining classification, snapshot, and runtime work—are defined in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation). v8.2.0 was published 2026-06-30 and includes Omens; observed development commits continued through 2026-08-03 [DATA-4]. The project warns that past releases are not back-patched and recommends pinning a tag [DATA-2].
...
Observed evidence attributes the exact `printed_name` `Gate to i'Arathael` to Card Vault base collector `IAR222` and the exact `name` `Gate to íArathael` to v8.2.0 `card.json` `IAR222`; exactly one of the 260 official names differs, and both strings are already NFC, so normalization cannot reconcile i-acute with apostrophe. Separately, captain-supplied product context names the product and next set **i'Arathael** and states that `IAR` denotes it. Future joins must remain collector-ID-first rather than name-only. The detailed observed IAR aggregates and captain-stated product/draft-legality boundary are recorded separately in [the rules and collation contract](rules-and-collation.md#iar-product-context-and-name-boundary).
