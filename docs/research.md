...
The captain downloaded `OMN_Draft_3.8 - Fixed New Layout Probabilities.txt` from the Rantaways server, which the captain records as widely used by players to practice draft, and expressly approved it as the MVP recipe [COMMUNITY-1]. This provenance is community evidence, not an LSS statement. The source remains outside the repository. Checksum verification, strict Settings-envelope and CustomCards-schema parsing, CustomCards aggregate validation, indentation-sensitive Layouts-schema parsing, and pinned Layouts aggregate enforcement exist; pool resolution/parsing, source reconciliation, probability derivation, generation, import, and snapshots remain unimplemented.
