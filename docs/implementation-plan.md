...
A test-first build-time importer reads pinned v8.2.0 inputs, the official Card Vault membership fixture, and the checksum-verified community 3.8 recipe. The implemented first public-card slice projects only exact source-order `set_id === "OMN"` identities and printings through the build-time-only schema-validation entry point, with pinned 251-card/482-printing/251-distinct-collector-ID aggregates. It does not establish official membership or IAR inclusion. Later slices reconcile 251 OMN + 9 IAR product entries, classify all treatments/slots/exclusions, map all 209 recipe card names/collector IDs to snapshot identities/treatments, and emit compact independently versioned set and visible-recipe snapshots. Card images remain URLs; embedded recipe URLs are not runtime authority.
