...
**Acceptance:** clean install is reproducible; baseline commands documented; CI/no-mistakes green; no product behavior yet; bundle tooling reports completed `apps/web/dist` bytes (with no client ceiling or size gate) and the zero-byte, not-yet-emitted server boundary.

## Phase 2 — Omens and community-recipe import

A test-first build-time importer will read pinned v8.2.0 inputs, reviewed official Card Vault membership evidence, and the checksum-verified community 3.8 recipe. Implemented public-card slices project exact source-order `set_id === "OMN"` identities and printings through the build-time-only schema-validation entry point, with pinned 251-card/482-printing/251-distinct-collector-ID aggregates, and retain the raw checksum of one caller-held 2026-08-04 Card Vault response as dated evidence. A separate strict build-time canonicalization establishes its 260-entry official membership (251 OMN + 9 IAR) independently of response serialization. All 260 validated exact bases are now joined to pinned upstream identities and all matching OMN and IAR printing rows. Current reconciliation scope, including retained uninterpreted `art_variations` metadata and the observed suffix-to-upstream-foiling correspondence, is specified in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation). Exact collector-first, derived pitch-name recipe-to-official identity reconciliation is complete through an opaque build-time capability, establishing 209 mapped and 51 `unmapped` identities. `unmapped` is not an exclusion or non-draftability classification. The following capability-bound rarity machinery implements the mapped-only observed-metadata contract specified in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation), but the real four-source result remains captain-acceptance-pending. Rarity is not itself slot, treatment, exclusion, draftability, probability, or runtime semantics. A separate bounded product-policy classification now marks the 209 mapped identities draftable, excludes only the nine unmapped IAR identities, and leaves the other 42 unmapped identities unclassified; its real four-source result is also captain-acceptance-pending. Validated pool references now resolve by exact same-source recipe ownership, then collector-first reconciliation, then exact draftable eligibility to ordered official identity facts without selecting physical forms; the real four-source pool result is captain-acceptance-pending. All validated weighted layout positions now preserve source order, repeated-reference multiplicity, and explicit recipe-structural common-rarity, fixed-rare, rare-or-majestic, and rainbow-foil roles while pointing to those exact opaque pools; these roles do not represent physical slots, and the real four-source layout result is captain-acceptance-pending. Exact source-order positive safe-integer cumulative-weight tables for all 228 layouts and 11 pools now compile through only those registered capabilities, preserving exact references and scoped totals; deterministic exact bounded-ticket lookup over only the registered capability returns its immutable layout or named-pool identity reference. One-sample composition now binds the engine's unbiased uint32 mapper to each exact layout or selected-pool total and returns immutable retry-or-selection only. A registered deeply immutable pack-local state now starts fresh from all 11 exact identity-pool tables and removes one exact selected identity only from its exact pool, recompiling source-order cumulative weights and the scoped total; dynamic bounded-ticket lookup selects one remaining immutable identity from that exact state and pool without sampling, retrying, removing, or sequencing. One uint32 sample now composes with that exact current state and pool, returning immutable retry-or-selection without removal, fallback, a retry loop, or sequencing. A separate atomic transition composes only that result with exact same-pool immutable removal, returning retry without identity or state and otherwise the exact selected identity reference plus the newly registered state. One selected layout now initializes a fresh immutable all-11-pool collation plan bound to that exact layout with cursor zero after validating all positions, roles, pool references, and same-pool no-replacement capacity. One finite caller-supplied uint32 batch now composes only the engine batch mapper, exact capability-bound layout ticket lookup, and that same registration into immutable need-more-or-plan output with the exact consumed count. A following finite batch now transitions exactly the current recipe-structural plan position and one identity draw through exact dynamic-pool selection, same-pool removal, and a new immutable cursor-advanced plan; need-more leaves the prior plan unchanged. Each plan retains an exact immutable source-order history of those accepted position and official-identity references, from an independently owned empty history through exactly 14 terminal records. This records selected identity references in a plan but constructs no pack or card instance, owns no entropy or caller retry policy, and decides no cross-pool duplicates. A separate build-time-only projection copies one exact caller-selected official identity, upstream printing, and Card Vault face into an immutable source-attributed presentation record; its evidence boundary is in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation). Complete finite-batch construction now sequences all 14 positions with immutable history/state and caller-owned entropy. It permits normal/Rainbow-Foil cross-pool identity overlap without deduplication or a policy switch; treatment distinguishes printings and pitch variants are distinct identities. The preceding real four-source results remain captain-acceptance-pending except the complete-construction result accepted on PR 66. A collation plan is not a pack, and recipe-structural positions are not physical slots. Card-presentation integration and selection, instances, inferred rears and treatment semantics, snapshots, and runtime integration remain future work. The implemented construction remains identity-only, and private four-source acceptance was accepted by the captain on PR 66 for exact head `aa825b18b5c455d253af9c0c45842b0b21a4d4bd` (20/20, green CI; squash `bfda2d97104312d4276523ff23f683b296747b22`). Suffix/treatment/art/face/URL semantics, physical-slot and treatment classification, all exclusion/draftability work beyond that partial identity fact, printing/treatment selection, image accessibility identity, rear markers, physical card-instance construction, rear slots, and simulation remain unimplemented. The reviewed versioned set snapshot is now generated, committed, and consumed by the browser client, which collates real packs from its weighted pools and layouts. The snapshot now carries each identity's exact official Card Vault `normal` rendition URL, and the browser requests that art directly from the pinned origin with no referrer, lazily, behind a content security policy naming only that origin, falling back to the card's name. Embedded recipe URLs are not runtime authority. The snapshot additionally carries exact upstream type tokens from the validated OMN source projection; its validator derives one card type and at most one class per identity from those tokens and refuses any unreviewed token, and the client groups a drafted pool by set number, class, colour, or type without changing it. A completed draft now offers the DT-7 Fabrary hand-off: the public pre-filled import link and the always-available copyable list. Re-checked 2026-08-06: the deep link still loads and selects Fabrary's Import tab, but a signed-out visitor is shown a sign-in wall instead of the import form, so pre-population could not be observed without an account and the fallback remains mandatory.
...
Integrate the implemented replay-only PCG and complete 14-position construction with conceptual 16-position wrapping/opaque rear removal, and extend the implemented pure draft transitions with product-specific timer, pause, cleanup, readiness, intermission, visibility, and instance semantics. Every non-host drafter seat vacancy, whether explicit voluntary leave or host removal, must atomically clear its provisional queue before replacement inheritance or fallback. Start each behavior with tests from [testing.md](testing.md#unit-and-property-matrix).
