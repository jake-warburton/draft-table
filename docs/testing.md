...
| Recipe parser | Deterministic Settings/CustomCards JSON plus indentation-sensitive Layout/pool parsing; required/unknown/duplicate section policy; 209 unique names/collector IDs; exact collector plus exact derived bare-name/pitch-color-suffix identity correspondence with no recipe-string rewriting; mapped rarity facts preserve exact recipe labels and upstream row-code sets, reject all unpinned mixed sets and aggregate redistribution, and do not gate 51 non-candidates; wrong/missing color, pitched pitchless names, bare pitched names, and all text drift fail; embedded image URLs are ignored as authority; malformed syntax/unknown names/non-positive weights fail closed. |
