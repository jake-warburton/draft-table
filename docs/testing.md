...
| Area | Required tests |
|---|---|
| Set import | Exact source checksums; 260 official product entries reconciled; 251 OMN + 9 IAR scope fixture; no duplicate stable IDs; exact recipe identity join is capability-bound and the real 209/51 partition remains four-source captain-acceptance-pending; every treatment classified; image host allowlist; unknown metadata fails closed. |
...
| Recipe parser | Deterministic Settings/CustomCards JSON plus indentation-sensitive Layout/pool parsing; required/unknown/duplicate section policy; 209 unique names/collector IDs; exact name-and-collector identity membership before later identity/treatment replacement; embedded image URLs are ignored as authority; malformed syntax/unknown names/non-positive weights fail closed. |
