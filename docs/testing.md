...
| Random bounds | One exact uint32 sample maps to an immutable accepted ticket in `[0,n)` or explicit retry through rejection sampling; exhaustive analogous-domain preimage checks prove no modulo bias. A finite caller-supplied uint32 batch validates every sample first, then composes the public mapper in source order and returns the first accepted ticket with exact consumption or explicit exhaustion. Build-time collation composition uses the exact capability-owned layout or selected-pool bound, propagates retry without lookup, and otherwise returns only the selected reference. Caller entropy ownership, generation, reseeding, infinite retry policy, set/layout/pool composition, plan transitions, pack construction, and runtime behavior remain later layers. |
