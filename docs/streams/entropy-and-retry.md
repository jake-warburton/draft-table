...
This is the replay helper's implementation description; the shared [domain model](../domain-model.md#replay-pcg-contract) and [accepted decisions](../risks-and-decisions.md#dt-8--replay-entropy-is-not-multiplayer-entropy) own its contract and production limitations.
...
The module remains intentionally unexported from `packages/engine/src/index.ts`; `drawDeterministicBoundedTicket` is an internal replay helper rather than a package-root API. The shared documents linked above record its accepted replay authorization. Production source ownership, stream/domain allocation, seed custody, secrecy controls, and server/runtime wiring remain undecided.
