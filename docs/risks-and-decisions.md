...
| Recipe/card-source mismatch | Recipe embeds 209 custom cards while official membership contains 260 entries and richer treatments. | Use the completed exact capability-bound identity join and staged same-source pool ownership resolution described in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation); require separate four-source captain acceptance before treating the real rarity correspondence, draft-eligibility classification, or pool identity resolution as established; embedded URLs never become runtime authority. |
