...
| Recipe/card-source mismatch | Recipe embeds 209 custom cards while official membership contains 260 entries and richer treatments. | Use the completed exact capability-bound identity join, staged same-source pool and weighted-layout resolution, and capability-only collation-weight compilation, bounded-ticket lookup, one-sample composition, pack-local same-pool state transition, dynamic state lookup, current-state one-sample composition, atomic same-pool sample selection-and-removal, layout-selected fresh collation-plan initialization, finite-batch collation-plan initialization, and finite-batch current-position transition described in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation); require separate four-source captain acceptance before treating any real capability result as established; embedded URLs never become runtime authority. |
