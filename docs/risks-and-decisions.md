...
| Recipe/card-source mismatch | Recipe embeds 209 custom cards while official membership contains 260 entries and richer treatments. | Use the completed exact capability-bound identity join and staged same-source pool and weighted-layout resolution described in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation); require separate four-source captain acceptance before treating the real rarity correspondence, draft-eligibility classification, pool identity resolution, or layout resolution as established; embedded URLs never become runtime authority. |
