...
| Recipe/card-source mismatch | Recipe embeds 209 custom cards while official membership contains 260 entries and richer treatments. | Use the completed exact capability-bound identity join, staged same-source pool and weighted-layout resolution, and capability-only collation-weight compilation and bounded-ticket lookup described in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation); require separate four-source captain acceptance before treating the real rarity correspondence, draft-eligibility classification, pool identity resolution, layout resolution, weight tables, or ticket lookup as established; embedded URLs never become runtime authority. |
