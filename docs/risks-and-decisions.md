...
External citation IDs resolve in the [research source register](research.md#source-register). The captain accepted the contracts below on 2026-08-03 and subsequently authorized Gate 0 and the walking-skeleton implementation slice. That authorization permits the isolated build-time public-card OMN source projection, dated raw-checksum verification of one caller-held observed Card Vault response, strict derivation of its canonical official membership fact, narrowly scoped identity/base reconciliation with the pinned public card source, retention of uninterpreted upstream `art_variations` metadata, observed official-suffix-to-upstream-foiling correspondence, and completed capability-bound exact recipe-to-official identity reconciliation. Authorization also extends to narrow observed recipe/upstream rarity correspondence machinery over only the 209 mapped identities; its real four-source result remains pending separate captain acceptance. The captain-approved bounded Omens product-policy eligibility fact classifies the 209 mapped identities as draftable, the nine unmapped IAR identities as excluded, and the other 42 unmapped identities as unclassified; its real four-source result also remains pending separate captain acceptance. Authorization extends to resolving validated private recipe pool references through exact same-source ownership and collector-first identity reconciliation to those draftable official identities, preserving all exact weighted layout positions as references to those opaque pools, compiling exact source-order integer cumulative-weight tables through those capabilities, deterministic exact bounded-ticket lookup over only the registered capability, one-sample unbiased uint32 composition to immutable retry-or-selection, registered pack-local repeated same-pool no-replacement transitions, dynamic bounded-ticket selection from that registered state, one-sample composition against its exact current pool total, atomic same-pool sample selection-and-removal, layout-selected fresh all-pool collation-plan initialization, finite caller-supplied batch initialization composed only from the engine mapper, exact layout ticket lookup, and that registration, one finite-batch current-position transition through exact dynamic-pool selection, same-pool removal, and an immutable cursor advance, and exact immutable source-order retention of each plan's accepted position and official-identity references, with all thirteen real four-source results still pending acceptance. Authorization now includes the replay-only deterministic PCG source and bounded retry transcript, complete finite-batch 14-position identity-only pack construction, legal normal-plus-Rainbow-Foil same-identity overlap without deduplication, capability-bound build-time presentation projection, the pure draft runtime, and fixture-only web playability. It does not extend these slices into production entropy, cryptographic source selection, stream allocation, presentation selection/integration, physical instances/rears, treatment semantics, snapshots, runtime room authority, or simulation.
...
### DT-8 — replay entropy is not multiplayer entropy
...
| Recipe/card-source mismatch | Recipe embeds 209 custom cards while official membership contains 260 entries and richer treatments. | Use the staged capability pipeline and separate four-source captain acceptance gates described in [card snapshot reconciliation](rules-and-collation.md#card-snapshot-reconciliation); embedded URLs never become runtime authority. |
...
| Replay PCG mistaken for secure entropy | Deterministic PCG output may be state-reconstructable. | Replay/known-answer use only; later architecture chooses separated and/or cryptographic production streams. |
